...
/**
 * `variant="rail"` drops the panel's own card, padding and title bar: docked in
 * the properties rail it is a list of groups inside somebody else's shell, and
 * two headers stacked on one panel is one header too many.
 */
export default function WorkflowMenu({ onClose, variant = 'panel' }: { onClose: () => void; variant?: 'panel' | 'rail' }) {
  ...
  return (
    <div
      style={{ padding: rail ? 0 : 20 }}
      className={rail
        ? 'workflow-menu w-full text-sm font-sans flex flex-col gap-3 select-none pointer-events-auto'
        : 'workflow-menu tool-panel max-w-[420px] w-full text-sm font-sans flex flex-col gap-4 max-h-[82vh] overflow-y-auto select-none pointer-events-auto'}
    >
      {/* Title — the rail's own header already says what this is. */}
      <div className={`flex items-center justify-between border-b border-[var(--border)] shrink-0 ${rail ? 'hidden' : ''}`} style={{ paddingBottom: 10 }}>
        <div>
